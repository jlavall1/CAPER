...
h.ppm(8) = uicontrol('style','popup','units','normalized',...
     'position',[0.037 0.006 0.235 0.106],'string',{'POI 1','POI 2','(2) PV POIs'},...
     'backgroundcolor',[0.973 0.973 0.973],'Fontsize',12); 
...
h.ppm(10) = uicontrol('style','popup','units','normalized',...
     'position',[0.351 0.006 0.235 0.106],'string',{'BASE Case','PV Case','BESS Case'},...
     'backgroundcolor',[0.973 0.973 0.973],'Fontsize',12); 
...
function m=p_run(varargin)
    ...
    if sim1_type == 1
        ...
        if QSTS_select == 4 %Impedance Location
            Location = get(h.ppm(8),'Value'); %Location=1 -> POI1, loc2=POI2, loc3=BOTH
        else
            Location = 0;
        end
    else
