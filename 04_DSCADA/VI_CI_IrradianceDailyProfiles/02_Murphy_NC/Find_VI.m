...
load M_TAYLOR.mat       %W/m^2;   kW_out; TEMP;   ELEV;   AZIM;    DOY;
    ...
    while DAY < MTH_LN(1,MNTH)+1
        while hr < 24
            while min < 60
                ...
                h = M_TAYLOR(MNTH).DAY(time2int(DAY,hr,min),4);    

                %extraterrestrial irradiance on a plane perp. to Sun's:
                n = M_TAYLOR(MNTH).DAY(time2int(DAY,hr,min),6);   
...
%                 B_ncI = AM*exp(-1*k*m);
                ...
                %GHI_k(i,2) = T_LI;
                %GHI_k(i,3) = G_hcI;
                M_TAYLOR(MNTH).GHI(time2int(DAY,hr,min),1) = B_ncI;
                M_TAYLOR(MNTH).GHI(time2int(DAY,hr,min),2) = T_LI;
                M_TAYLOR(MNTH).GHI(time2int(DAY,hr,min),3) = G_hcI;
                %Inc. Variables:
                min = min + 1;
                i = i + 1;
            end
...
while MNTH < 13
    while DAY < MTH_LN(1,MNTH)+1
        while hr < 24
            while min < 60
                %Measured Quantities:
                GHI_k = M_TAYLOR(MNTH).DAY(time2int(DAY,hr,min),1);
                GHI_k1 = M_TAYLOR(MNTH).DAY(time2int(DAY,hr,(min-1)),1);
                %Calculated Quantities:
                CSI_k = M_TAYLOR(MNTH).GHI(time2int(DAY,hr,min),3);
                CSI_k1 = M_TAYLOR(MNTH).GHI(time2int(DAY,hr,(min-1)),3);
                B_nck = M_TAYLOR(MNTH).GHI(time2int(DAY,hr,min),1);
                if GHI_k ~= 0 && CSI_k ~= 0
                    %Find VI:
...
MNTH = 1;
plot(M_TAYLOR(MNTH).DAY(time2int(D_S,0,0):time2int(D_S,23,59),1),'r--');
hold on
plot(M_TAYLOR(MNTH).GHI(time2int(D_S,0,0):time2int(D_S,23,59),3),'b-');
hold on
plot(M_TAYLOR(MNTH).GHI(time2int(D_S,0,0):time2int(D_S,23,59),1),'g-');
...
%%
%Find samples days of VI 1 to 20
i = 1;
n = 1;
HOLD = zeros(1,2);% VI | i
DIFF = 10;
SAMPLES_T = zeros(2,20);
HOLD(1,1) = 100;
while n < 26
    while i < 366
...
while n < 21
    ...
    hold on
    %Calc. Cleark Sky:
    plot(M_TAYLOR(MNTH).GHI(time2int(D_S,0,0):time2int(D_S,23,59),3),'r-');
