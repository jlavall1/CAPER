...
%  For 1day at 1min resolution then nstp = 1440; step = 60;
date = '12/31/2014';
nstp = 24; % Number of steps
step = 3600;   % [in secs] - Resolution of step
...
filename = '09_TOW\Mocks_01\Peak_shave\Master.dss';
% ******To skip UIGETFILE uncomment desired filename*******
...
res   = 3600;           % [s] - Resolution of data
ndat  = 24;       % Number of Data Points
...
%         sprintf('%02d',mod(floor(index(i)*res/60),60))];
    ...
    %DATA(i).VoltagePhaseA = MOCKS01.Voltage.A(index(i));
    %DATA(i).VoltagePhaseB = MOCKS01.Voltage.B(index(i));
    %DATA(i).VoltagePhaseC = MOCKS01.Voltage.C(index(i));
    
    DATA(i).CurrentPhaseA = MOCKS01.Amp.A(index(i));
    DATA(i).CurrentPhaseB = MOCKS01.Amp.B(index(i));
    DATA(i).CurrentPhaseC = MOCKS01.Amp.C(index(i));
    
    DATA(i).RealPowerPhaseA = MOCKS01.kW.A(index(i));
    DATA(i).RealPowerPhaseB = MOCKS01.kW.B(index(i));
    DATA(i).RealPowerPhaseC = MOCKS01.kW.C(index(i));
    
    DATA(i).ReactivePowerPhaseA = MOCKS01.kVAR.A(index(i));
    DATA(i).ReactivePowerPhaseB = MOCKS01.kVAR.B(index(i));
    DATA(i).ReactivePowerPhaseC = MOCKS01.kVAR.C(index(i));
end
%clear CMNWLTH
