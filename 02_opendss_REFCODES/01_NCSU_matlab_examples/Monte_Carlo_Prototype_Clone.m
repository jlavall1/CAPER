...
%Compile the circuit
DSSText.command = 'compile C:\Users\jlavall\Documents\00_CAPER_SYSTEM\05_OpenDSS_Circuits\HollySprings_Circuit_Opendss\Run_Master_Allocate.DSS';
...
for i=1:length(Buses);
    ...
    if (Monte_Carlo_Random >0.5)
        ...
        DSS_Command = sprintf('New PVSystem.%s_PV phases=%s bus1=%s kV=%s kVA=500 irrad=0.8 Pmpp=500 temperature=25 PF=1 effcurve=MyEff P-TCurve=MyPvsT Daily=MyIrrad TDaily=MyTemp',Active_Bus.name,Active_Bus_phases,Active_Bus.name,Active_Bus_voltage);
